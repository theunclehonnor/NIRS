...
class ProductController extends AbstractController
{
    ...
    /**
     * @Route("product/my", name="product_my", methods={"GET"})
     */
    public function productMy(ProductRepository $productRepository): Response
    {
        if(strcasecmp('ROLE_PROVIDER',$this->getUser()->getRoles()[0]) != 0) {
            return $this->redirectToRoute('product_index');
        }
        $allProducts = $productRepository->findBy(['_user' => $this->getUser()], ['create_at' =>'DESC']);
        foreach ($allProducts as $product) {
            $temp = $this->generateTotal($product->getId());
            $product->setAvarageEstimate($temp[0]);
            $product->setLenComments($temp[1]);
        }
        return $this->render('product/my.html.twig', [
            'products' => $allProducts
        ]);
    }
